...
void mostrarAlumno(char nombre[], int leg, char sexo, int nota1, int nota2, float promedio);

void ordenarAlumnos(int leg[], char names[][20], char sex[], int n1[], int n2[], float prom[], int tam);
...
int main()
{
    ...
    ordenarAlumnos(legajos, nombres, sexos, notas1, notas2, promedios, TAM);
...
void mostrarAlumnos(int leg[], char names[][20], char sex[], int n1[], int n2[], float prom[], int tam)
{
    ...
    for(int i=0;i<tam;i++)
    {
        ...
        mostrarAlumno(names[i], leg[i], sex[i], n1[i], n2[i], prom[i]);
    }
...
void ordenarAlumnos(int leg[], char names[][20], char sex[], int n1[], int n2[], float prom[], int tam)
{
    ...
    for(int i=0;i<tam;i++)
    {
        ...
        {
            if(sex[i]<sex[j] || (sex[i]==sex[j] && leg[i]>leg[j]) )
            {
                auxFloat = prom[i];
                prom[i] = prom[j];
                prom[j] = auxFloat;
...
void mostrarAlumno(char nombre[], int leg, char sexo, int nota1, int nota2, float promedio)
{
    printf("  %6d    %7s   %c          %2d               %2d             %5.2f\n", leg, nombre, sexo, nota1 , nota2, promedio);
}
